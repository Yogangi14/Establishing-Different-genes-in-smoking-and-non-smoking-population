...
mars_data = PrepareDF("C:/Users/yogan/Downloads/01_data_mars_opposition_updated.csv")
...
def bestOrbitInnerParams(r,
                         ...
                         oppositions=longitude_series):
    counter = 20
    # Initialization of variable s and r
    opti_args['s'] = s
    opti_args['r'] = r
## helper function run_exhaustive
...
def bestS(r, args=q3_args, times=time_series, oppositions=longitude_series):
    ...
    res = minimize(objective_3,
                   ...
                   method='Nelder-Mead',
                   tol=1e-16,
                   bounds=[(0.97 * 0.00918694657579844,
                            1.03 * 0.00918694657579844)])
    q3_args['s'] = res.x[0]
...
## wrapper code 
## QUESTion 5
def bestMarsOrbitParams(times, oppositions, calculated_guess):
    """
    Wrapper function to iteratively search for the best r, s, c, e1, e2, and z parameters
    that minimize the maximum angular error for each opposition.
    
    Parameters:
    - times: array of times for oppositions in days
    - oppositions: array of observed opposition longitudes in degrees
    - initial_guess: initial guess for the parameters (r, s, c, e1, e2, z)

    Returns:
    - Best parameters (r, s, c, e1, e2, z)
    - Errors for each opposition
    - Maximum angular error
    """
    # Unpacking the initial guess
    r, s, c_init, e1_init, e2_init, z_init = calculated_guess

    # Function to optimize r and s while fitting other parameters
    def objective_rs(params):
        r_opt, s_opt = params
        c_opt, e1_opt, e2_opt, z_opt, errors, max_error = bestOrbitInnerParams(
            r_opt, s_opt, opti_args={
                'c': c_init,
                'e1': e1_init,
                'e2': e2_init,
                'z': z_init
    ...
    # Initial bounds for r and s (you can adjust them)
    bounds = [(0.97 * r, 1.03 * r), (0.97 * s, 1.03 * s)]

    # Optimization of r and s
    result = minimize(objective_rs, x0=[r, s], method='Nelder-Mead', bounds=bounds, tol=1e-16)

    # Extract the optimized r and s
    r_opt, s_opt = result.x

    # Now, with optimized r and s, fit the other parameters (c, e1, e2, z)
    c_opt, e1_opt, e2_opt, z_opt, errors, max_error = bestOrbitInnerParams(
        r_opt, s_opt, opti_args={
            'c': c_init,
            'e1': e1_init,
    ...
    return r_opt, s_opt, c_opt, e1_opt, e2_opt, z_opt, errors, max_error



# Initi guess from the individual guesses r, s, c, e1, e2, z
calculated_guess = [7.965989664867674, 0.009147097798936077, 3.0358031262583065,1.4777442341378864 ,2.5748354680546046, 0.9685872374580766]

# Call the function
r, s, c, e1, e2, z, errors, max_error = bestMarsOrbitParams(time_series, longitude_series, calculated_guess)
...
## orbit plot
mars_orbit = MarsModel(c, r, e1, e2, z, s)

# Call the plot function to visualize the orbit, equant, and Sun
mars_orbit.plot()
